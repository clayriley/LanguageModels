...
public class LanguageModelTester {
  ...
  static double computePerplexity(LanguageModel languageModel, 
    ...
    float logProbability = (float) 0.0;
    double numSymbols = 0.0;
    for (List<String> sentence : sentences) {
      logProbability += 
        languageModel.getSentenceLogProbability(sentence) / 
        Math.log(2.0);
  ...
  static Pair<Double,Double> computeWordErrorRate(LanguageModel languageModel, 
    ...
    if( showGuesses ) System.out.println("***Rebuilt Jumbled sentences***");
    ...
    for (JumbleProblem jProblem : jumbleProblems) {
    	...
    	for (List<String> guess : jProblem.getNBestSentences()) {
    		double score = languageModel.getSentenceLogProbability(guess);
   ...
   * each jumbled sentence problem, the candidate answer with the
   ...
   * each jumbled sentence problem, a random candidate answer is
  ...
  /** 
   * Takes a list of jumbled sentence problems, extracts the correct
  ...
  static Collection<List<String>> getCorrectSentences(List<JumbleProblem> jumbleProblems) {
	...
	private static void evaluateContext(LanguageModel model, String contextString){
		...
		cs114.util.PriorityQueue<String> pq = wordProbabilities.asPriorityQueue();
  ...
  /**
   * The main method loads language model training, dev, and test
   * data files, along with files containing a set of jumbled sentences from
   * the jumbled sentence corpus.  It trains the language model using the training
   * data, and then computes the perplexity of the test data w.r.t. the
   * model, the perplexity of the correct answers to the jumbled sentence problems
  ...
  public static void main(String[] args) throws IOException {
    ...
    options.put("-train",     "train-data.txt");
    options.put("-dev",     "dev-data.txt");
    options.put("-test",      "test-data-no-oov.txt");
    options.put("-model",     "cs114.langmodel.ExampleUnigram");

    options.put("-showguesses",  "false");  // show rebuilt jumbled sentence emails?
    options.put("-jumble",       "true");  // run Jumble (jumbled sentence) evaluation?
    ...
    String devFile = dataPath + "/" + options.get("-dev");
    ...
    System.out.println("Validation data will be read from " + devFile);
    Collection<List<String>> devSentences = Sentences.Reader.readSentences(devFile);
    ...
    if ("true".equals(options.get("-jumble"))) {
      System.out.printf("%-30s","Jumbled sentences: True answer perplexity: ");
	...
	showGuesses = true;
      ...
      System.out.printf("%-30s","Jumbled sentences: Word Error Rate: ");
      System.out.println(nf.format(results.getFirst()));
      System.out.printf("%-30s","Jumbled sentences: Percent Correct: ");
      System.out.println(nf.format(100*results.getSecond()) + "%");

      if ("true".equals(options.get("-baselines"))) {
        System.out.println();
        System.out.println("Jumbled sentences WER Baselines: ");
